...
export default function Page() {
  ...
  useEffect(() => {
    setEmail(
      searchParams.get("email") ||
        sessionStorage.getItem("nftcunion_verification_email") ||
        "",
    );
  ...
  async function verify() {
    ...
    try {
      ...
      if (response.ok) {
        sessionStorage.removeItem("nftcunion_verification_email");
  ...
  return (
    <main className="relative grid min-h-screen place-items-center bg-white px-5 py-20">
      ...
      {loading && (
        <div className="fixed inset-0 z-[200] bg-[#0b1f3a]/20 backdrop-blur-[1px]">
