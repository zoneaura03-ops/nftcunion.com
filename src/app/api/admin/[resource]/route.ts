  ...
  try {
    ...
    if (resource === "users") {
      if (
        ...
        await connection.execute(
          "INSERT INTO notifications(user_id,type,title,body) VALUES(?,'account','Account created','An administrator created and activated your North Fountain Trust Credit Union account.')",
          [subject],
        );
      } else {
      ...
      if (rows[0]?.user_id)
        await connection.execute(
          ...
          [
            ...
            approved
              ? "Your grant application has been approved. A North Fountain Trust Credit Union representative will contact you with the next steps."
