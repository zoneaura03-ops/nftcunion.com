...
export async function GET(request: Request) {
  ...
  if (!number || !/^\d{8,20}$/.test(number))
    return NextResponse.json(
      { error: "Enter a valid North Fountain Trust Credit Union account number." },
      { status: 400 },
    );
  ...
  if (!row)
    return NextResponse.json(
      { error: "The North Fountain Trust Credit Union account was not found." },
      { status: 404 },
    );
  return NextResponse.json({
    recipient: {
      ...
      bankName: process.env.BANK_LEGAL_NAME || "North Fountain Trust Credit Union",
