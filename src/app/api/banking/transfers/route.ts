  ...
  try {
    ...
    if (type === "internal") {
      ...
      if (!destination)
        throw new BankingError(
          "The North Fountain Trust Credit Union recipient account was not found.",
          404,
        );
      if (Number(destination.user_id) === user.id)
        throw new BankingError("Choose another North Fountain Trust Credit Union account.", 422);
      if (destination.currency !== account.currency)
        throw new BankingError(
          "Internal transfers require matching account currencies.",
          422,
        );
      const bankName = process.env.BANK_LEGAL_NAME || "North Fountain Trust Credit Union";
      if (
    ...
    const [transferResult] = await connection.execute(
      ...
      [
        ...
        type === "internal"
          ? process.env.BANK_LEGAL_NAME || "North Fountain Trust Credit Union"
          : walletProviderNames[type] || body.bankName || "North Fountain Trust Credit Union",
