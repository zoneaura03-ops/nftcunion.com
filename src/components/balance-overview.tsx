...
export function BalanceOverview({
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("nftcunion-balance-visible");
  ...
  function toggle() {
    setVisible((current) => {
      const next = !current;
      window.localStorage.setItem("nftcunion-balance-visible", String(next));
      return next;
    });
  }

  return (
    <section className="relative overflow-hidden rounded-[22px] bg-gradient-to-br from-[#0b1f3a] via-[#10233a] to-[#07111f] text-white shadow-[0_22px_55px_rgba(10,23,40,.22)]">
      <div className="absolute -left-20 -top-24 size-64 rounded-full bg-[#d8b45b]/10" />
