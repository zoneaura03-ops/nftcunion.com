      ...
      const safeDestination =
        requestedDestination?.startsWith("/dashboard/") && data.role !== "admin"
          ...
          : data.role === "admin"
            ? "/admin"
            : "/dashboard";
      sessionStorage.setItem("nftcunion_login_destination", safeDestination);
      router.push("/pin");
    } catch (reason) {
      ...
      <Frame>
        ...
        <form onSubmit={submit} className="mt-7 space-y-5">
          ...
          {notice && (
            <p
              role="status"
              className="rounded-md border border-gold-300 bg-gold-50 p-3 text-sm text-[#0b1f3a]"
            >
              {notice}
            </p>
          )}
