    ...
    return (
      <div className="mx-auto max-w-2xl">
        ...
        <div className="card rounded-2xl p-6 sm:p-8">
          ...
          <Review rows={[["From", `${account?.account.name || "North Fountain Trust Credit Union account"} · ${maskedAccount}`], ["Provider", details.name], ["Recipient", recipientName], [details.identifier, maskIdentifier(identifier, provider)], ...(provider === "paypal" ? [["PayPal payment type", paymentType === "goods_services" ? "Goods and services" : "Friends and family"]] : []), ["Purpose", purpose], ["Note", note || "None"], ["Amount", money(numericAmount, account?.account.currency)], ["Fee", money(0, account?.account.currency)], ["Total debit", money(numericAmount, account?.account.currency)]]} />
