              ...
              <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50/60 p-5">
                {messages.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`max-w-[85%] rounded-2xl border p-3 text-sm shadow-sm ${admin ? "ml-auto border-[#0b1f3a] bg-[#0b1f3a] text-white" : bot ? "border-violet-200 bg-violet-50 text-violet-950" : "border-sky-200 bg-sky-50 text-sky-950"}`}
                    >
                      <p
