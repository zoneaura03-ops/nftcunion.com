...
export default function AdminTransferQueue() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <section className="relative overflow-hidden rounded-[28px] bg-[#0b1f3a] p-7 text-white shadow-[0_22px_65px_rgba(10,23,40,.17)] sm:p-9">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button type="button" onClick={load} className="grid size-12 place-items-center rounded-2xl border border-white bg-white shadow-lg" aria-label="Refresh transfers"><RefreshCw size={20} strokeWidth={2.5} color="#0b1f3a" /></button>
        </div>
      </section>
          ...
          return (
            <article key={id} className="overflow-hidden rounded-3xl border border-[#dfe5ef] bg-white shadow-[0_12px_35px_rgba(10,23,40,.06)]">
              <div className="grid gap-5 border-b border-[#e2e7f0] p-5 sm:p-6 lg:grid-cols-[1.3fr_.8fr_auto] lg:items-center">
                <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-bank-50 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-bank-700">{String(row.transfer_type || "transfer")}</span><span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${verified ? "bg-gold-50 text-[#0b1f3a]" : "bg-amber-50 text-amber-700"}`}>{stageLabels[String(row.verification_stage)] || String(row.verification_stage || "Pending")}</span></div><h2 className="mt-3 text-xl font-bold">{String(row.recipient_name || "Recipient")}</h2><p className="mt-1 text-xs text-neutral-500">{String(row.email)} · {String(row.reference)}</p></div>
                <div><p className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Transfer amount</p><p className="mt-2 text-2xl font-bold text-[#0b1f3a]">{amount} {String(row.currency || "")}</p><p className="mt-1 text-xs text-neutral-500">{String(row.bank_name || "North Fountain Trust Credit Union")} · {String(row.created_at || "")}</p></div>
