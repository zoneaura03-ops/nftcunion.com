...
export function FeaturePage({ feature }: { feature: string }) {
  ...
  return (
    <Generic
      ...
      copy="Manage this service securely from your North Fountain Trust Credit Union account."
...
function TransferFlow({
  ...
  const title =
    type === "international"
      ...
      : type === "internal"
        ? "Internal North Fountain Trust Credit Union Transfer"
        : "Local Bank Transfer";
  if (step === "processing")
    return (
      <StateCard
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm sm:p-8">
          ...
          <Review
            rows={[
              ...
              ["Bank", type === "internal" ? "North Fountain Trust Credit Union" : form.bankName],
              [
                "Amount",
      ...
      <Header
        ...
        copy={
          type === "international"
            ...
            : type === "internal"
              ? "Send instantly to another North Fountain Trust Credit Union account."
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-[#e1e6ef] bg-white">
          {!data ? <RowsLoading /> : data.swaps.length ? data.swaps.map((swap) => (
            <article key={swap.reference} className="flex flex-col gap-3 border-b p-5 last:border-0 sm:flex-row sm:items-center sm:justify-between">
              <div><div className="flex items-center gap-2"><b>{swap.from_asset} â†’ {swap.to_asset}</b><span className="rounded-full bg-gold-50 px-2 py-1 text-[9px] font-bold uppercase text-[#0b1f3a]">{swap.status}</span></div><p className="mt-1 text-xs text-neutral-500">{swap.reference} Â· {new Date(swap.created_at).toLocaleString("en-GB")}</p></div>
    ...
    <div className="receipt-page mx-auto max-w-xl">
      <div className="receipt-paper relative overflow-hidden rounded-2xl border bg-white p-7 shadow-sm">
        <div className="relative">
          <div className="flex items-center justify-between border-b pb-5"><div className="flex items-center gap-3"><BrandMark className="h-9 w-9" /><div><p className="text-xs font-bold tracking-[.22em] text-bank-700">NORTH FOUNTAIN <span className="text-gold-600">TRUST CREDIT UNION</span></p><p className="mt-1 text-[10px] uppercase tracking-widest text-neutral-400">Transaction receipt</p></div></div><span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${String(item.status).toLowerCase().startsWith("declin")||String(item.status).toLowerCase()==="failed"?"bg-red-50 text-red-700":"bg-emerald-50 text-emerald-700"}`}>{String(item.status)}</span></div>
        ...
        <Review
          rows={[
            ...
            ["Sender", String(item.sender_first_name ? `${item.sender_first_name} ${item.sender_last_name}` : item.type === "debit" ? `${item.first_name} ${item.last_name}` : "North Fountain Trust Credit Union")],
            ["Sender account", String(item.sender_account || (item.type === "debit" ? item.account_number : "North Fountain Trust Credit Union"))],
  ...
  return (
    <div>
      <Header
        eyebrow="Cards"
        title="Virtual cards"
        copy="Apply for a Visa, Mastercard, or North Fountain Trust Credit Union Credit Card, then manage it securely."
      />
      ...
      <section className="mt-6 rounded-2xl border border-[#e1e6ef] bg-white p-5 shadow-[0_8px_24px_rgba(10,23,40,.05)] sm:p-6">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {([
            ...
            ["amex", "North Fountain Trust Credit Union credit card", "Apply for credit"],
              ...
              >
                ...
                <span className="flex items-center gap-3 px-1 pb-1 pt-4">
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-2"><span className="block truncate font-bold">{title}</span>{owned && <span className="rounded-full bg-gold-100 px-2 py-1 text-[9px] font-bold uppercase text-[#0b1f3a]">Owned</span>}{pendingApplication && <span className="rounded-full bg-amber-100 px-2 py-1 text-[9px] font-bold uppercase text-amber-700">Under review</span>}</span>
                  ...
                  <div className="grid min-w-0 gap-5 sm:grid-cols-[260px_1fr] sm:items-center lg:grid-cols-[260px_1fr_auto]">
                    ...
                    <div className="flex items-end justify-between gap-4 border-t border-neutral-100 pt-4 sm:col-span-2 lg:col-span-1 lg:block lg:min-w-40 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0 lg:text-right">
                      <div>
                        ...
                        <p className={`mt-1 font-bold ${active ? "text-xl text-[#111827]" : pending ? "text-amber-700" : "text-[#0b1f3a]"}`}>
        ...
        >
          <section className="w-full max-w-4xl rounded-3xl bg-white p-5 shadow-[0_30px_90px_rgba(0,25,15,.3)] sm:p-7">
            ...
            <p className="mt-2 text-sm leading-6 text-neutral-500">
              Your application will be sent to a North Fountain Trust Credit Union administrator for review. Card details and funding controls become available after approval.
            </p>
            {error && (
              <div role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                <p>{error}</p>
                {verificationRequired && (
                  <Link href="/dashboard/verification" className="mt-3 inline-flex font-bold text-[#0b1f3a] underline underline-offset-4">
...
function GrantApplications() {
  ...
  useEffect(() => {
    const recovered = sessionStorage.getItem("nftcunion_grant_recovery");
    if (recovered) {
      try {
      ...
      sessionStorage.removeItem("nftcunion_grant_recovery");
    }
    loadGrants();
  }, []);
  ...
  function recoverExpiredSession() {
    sessionStorage.setItem(
      "nftcunion_grant_recovery",
      JSON.stringify({ form, step, draftId }),
    );
    sessionStorage.setItem("nftcunion_login_destination", "/dashboard/grants");
              ...
              <div>
                ...
                <div className="mt-6 space-y-5">
                  ...
                  <label className="flex items-start gap-3 rounded-xl border p-4 text-sm">
                    ...
                    <span><b className="block">Declaration and consent</b><span className="mt-1 block text-xs leading-5 text-neutral-500">I declare that the information is accurate and consent to North Fountain Trust Credit Union reviewing the application and supporting documents.</span></span>
                  </label>
                </div>
              </div>
            )}
    ...
    <div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,.8fr)]">
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl bg-[#0b1f3a] p-6 text-white"><ShieldCheck className="text-gold-300" /><h3 className="mt-4 text-xl font-bold">Your security matters</h3><p className="mt-2 text-sm leading-6 text-gold-50/75">North Fountain Trust Credit Union support will never ask for your password, full card number, CVV, or transaction PIN.</p></div>
  ...
  return (
    <div>
      <Header
        eyebrow="Wealth"
        title="Investments"
        copy="Build a diversified portfolio directly from your North Fountain Trust Credit Union account."
      />

      <section className="relative mt-6 overflow-hidden rounded-3xl bg-gradient-to-br from-[#101f33] via-[#0d1c30] to-[#0b1f3a] p-6 text-white shadow-[0_22px_60px_rgba(10,23,40,.20)] sm:p-8">
              ...
              <article key={holding.id} className="flex flex-col gap-4 border-b p-5 last:border-0 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-gold-50 px-2 py-1 text-[9px] font-bold uppercase text-[#0b1f3a]">{holding.status}</span>
                  </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[#e2e7f0] p-4 text-sm">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-neutral-500">
                  Receive optional North Fountain Trust Credit Union product news.
                </span>
              </span>
            </label>
          </div>
